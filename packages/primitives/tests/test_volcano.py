"""Tests for the volcano primitive (figure emission, raw matplotlib)."""
...
import lattice_primitives.plot.volcano  # noqa: F401  (register)
...
def test_volcano_emits_figure_default_group(adata_log_normalized: AnnData) -> None:
    adata = _through_markers(adata_log_normalized)
    # group=None -> first group in the result
    adata_out, _, result = run_primitive("volcano", adata, group=None)
    assert len(result.figures) == 1
    assert result.figures[0].startswith("data:image/png;base64,")
    assert "_lattice_figures" not in adata_out.uns


def test_volcano_requires_rank_genes_groups(adata_log_normalized: AnnData) -> None:
    ...
    with pytest.raises(ValueError, match="rank_genes_groups"):
        run_primitive("volcano", out, group=None)


def test_volcano_rejects_unknown_group(adata_log_normalized: AnnData) -> None:
    adata = _through_markers(adata_log_normalized)
    with pytest.raises(ValueError, match="not found in rank_genes_groups"):
        run_primitive("volcano", adata, group="not_a_real_group")
